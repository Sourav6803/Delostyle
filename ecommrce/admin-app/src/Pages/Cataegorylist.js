...
const Categorylist = () => {
    ...
    for (let i = 0; i < pCatStat.length; i++) {
        data1.push({
            ...
            action: (<>
                <Link to={`/admin/category/${pCatStat[i]._id}`} className='fs-3 text-danger'> <BiEdit /></Link>
                <button onClick={()=>showModal(pCatStat[i]._id)} className='ms-3 fs-3 text-danger bg-transparent border-0'> <AiFillDelete /></button>
            </>)
        });
    }
    ...
    return (
        <div >
            <h3 className='mb-4 title'>Category Lists</h3>
            <div>
